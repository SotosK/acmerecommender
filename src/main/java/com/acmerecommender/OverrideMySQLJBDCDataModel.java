 ...
 * </p>
 */
public class OverrideMySQLJBDCDataModel extends AbstractJDBCDataModel {
     ...
     */
    public OverrideMySQLJBDCDataModel() throws TasteException {
     ...
     *           if {@link DataSource} can't be found
     */
    public OverrideMySQLJBDCDataModel(String dataSourceName) throws TasteException {
     ...
     */
    public OverrideMySQLJBDCDataModel(DataSource dataSource) {
        this(dataSource,
                DEFAULT_PREFERENCE_TABLE,
                DEFAULT_USER_ID_COLUMN,
                DEFAULT_ITEM_ID_COLUMN,
    ...
    public OverrideMySQLJBDCDataModel(DataSource dataSource,
                                      ...
                                      String userIDColumn,
                                      String itemIDColumn,
                                      String userAccountIDColumn,
                                      int userAccountID,
                                      String preferenceColumn,
                                      String timestampColumn) {
        super(dataSource, preferenceTable, userIDColumn, itemIDColumn, preferenceColumn,
                // getPreferenceSQL
                "SELECT " + preferenceColumn + " FROM " + preferenceTable + " WHERE " + userAccountIDColumn + "=" + userAccountID + " AND " + userIDColumn + "=? AND "
                         + itemIDColumn + "=?",
                // getPreferenceTimeSQL
                "SELECT " + timestampColumn + " FROM " + preferenceTable + " WHERE " + userAccountIDColumn + "=" + userAccountID + " AND " + userIDColumn + "=? AND "
                         + itemIDColumn + "=?" ,
                // getUserSQL
                "SELECT DISTINCT " + userIDColumn + ", " + itemIDColumn + ", " + preferenceColumn + " FROM " + preferenceTable
                        + " WHERE " + userAccountIDColumn + "=" + userAccountID + " AND " + userIDColumn  + "=? ORDER BY " + itemIDColumn,
                // getAllUsersSQL
                "SELECT DISTINCT " + userIDColumn + ", " + itemIDColumn + ", " + preferenceColumn + " FROM " + preferenceTable + " WHERE " + userAccountIDColumn + "=" + userAccountID
                        + " ORDER BY " + userIDColumn + ", " + itemIDColumn,
                // getNumItemsSQL
                "SELECT COUNT(DISTINCT " + itemIDColumn + ") FROM " + preferenceTable+ " WHERE " + userAccountIDColumn + "=" + userAccountID,
                // getNumUsersSQL
                "SELECT COUNT(DISTINCT " + userIDColumn + ") FROM " + preferenceTable+ " WHERE " + userAccountIDColumn + "=" + userAccountID,
                // setPreferenceSQL
                "INSERT INTO " + preferenceTable+ " WHERE " + userAccountIDColumn + "=" + userAccountID + '(' + userIDColumn + ',' + itemIDColumn + ',' + preferenceColumn
                        + ") VALUES (?,?,?) ON DUPLICATE KEY UPDATE " + preferenceColumn + "=?",
                // removePreference SQL
                "DELETE FROM " + preferenceTable + " WHERE " + userIDColumn + "=? AND "+ userAccountIDColumn + "=" + userAccountID + " AND " + itemIDColumn + "=?",
                // getUsersSQL
                "SELECT DISTINCT " + userIDColumn + " FROM " + preferenceTable+ " WHERE " + userAccountIDColumn + "=" + userAccountID + " ORDER BY " + userIDColumn,
                // getItemsSQL
                "SELECT DISTINCT " + itemIDColumn + " FROM " + preferenceTable+ " WHERE " + userAccountIDColumn + "=" + userAccountID + " ORDER BY " + itemIDColumn,
                // getPrefsForItemSQL
                "SELECT DISTINCT " + userIDColumn + ", " + itemIDColumn + ", " + preferenceColumn + " FROM " + preferenceTable
                        + " WHERE "+ userAccountIDColumn + "=" + userAccountID + " AND " + itemIDColumn + "=? ORDER BY " + userIDColumn,
                // getNumPreferenceForItemSQL
                "SELECT COUNT(1) FROM " + preferenceTable + " WHERE " + userAccountIDColumn + "=" + userAccountID + " AND " + itemIDColumn + "=?",
                // getNumPreferenceForItemsSQL
                "SELECT COUNT(1) FROM " + preferenceTable +  " tp1 JOIN " + preferenceTable + " tp2 " + "USING ("
                        + userIDColumn + ") WHERE tp1."+ userAccountIDColumn + "=" + userAccountID + " AND tp1." + itemIDColumn + "=? and tp2." + itemIDColumn + "=?",
                "SELECT MAX(" + preferenceColumn + ") FROM " + preferenceTable + " WHERE " + userAccountIDColumn + "=" + userAccountID,
                "SELECT MIN(" + preferenceColumn + ") FROM " + preferenceTable + " WHERE " + userAccountIDColumn + "=" + userAccountID);
    }
